...
_log = storjlib.log.getLogger(__name__)
_builtin_open = open
...
def _get_shard_path(store_path, shardid, use_folder_tree,
    ...
    if use_folder_tree:
        folders = os.path.join(*storjlib.util.chunks(shardid, 3))
        store_path = os.path.join(store_path, folders)
        if create_needed_folders:
            storjlib.util.ensure_path_exists(store_path)
...
def setup(store_config):
    ...
    Example:
        ...
        normalized_paths = storjlib.store.manager.setup(store_config)
    """
    normal_paths = {}
    for path, attributes in store_config.items():
        ...
        # check path
        path = storjlib.util.full_path(path)
        storjlib.util.ensure_path_exists(path)

        # check limit
        limit = storjlib.util.byte_count(attributes.get("limit", 0))
        free = storjlib.util.get_free_space(path)
        used = storjlib.util.get_folder_size(path)
        available = (free + used)
        if limit > available:
        ...
        if not use_folder_tree and storjlib.util.get_fs_type(path) == "vfat":
...
def open(store_config, shardid):  # FIXME require config instead
    ...
    Example:
        ...
        with storjlib.store.manager.open(store_config, id) as shard:
            print(storjlib.store.shard.get_id(shard)
    """
...
def capacity(store_config):  # FIXME require config instead
    ...
    Example:
        import storjlib
        store_config = {"path/alpha": None, "path/beta": None}
        print(storjlib.store.manager.capacity(store_config))
    """
    ...
    for store_path, attributes in store_config.items():
        free_disc_space = storjlib.util.get_free_space(store_path)
        limit = attributes["limit"] or free_disc_space
        path_used = storjlib.util.get_folder_size(store_path)
        total += limit
        used += path_used
        free += limit - used
...
def add(store_config, shard):  # FIXME require config instead
    ...
    Example:
        ...
        with open("path/to/loose/shard", "rb") as shard:
            storjlib.storage.manager.add(store_config, shard)
    """
    store_config = setup(store_config)  # setup if needed
    shardid = storjlib.store.shard.get_id(shard)
    shard_size = storjlib.store.shard.get_size(shard)
    ...
    for store_path, attributes in items:
        ...
        limit = storjlib.util.byte_count(attributes["limit"])
        used = storjlib.util.get_folder_size(store_path)
        free = limit - used
        ...
        free_space = storjlib.util.get_free_space(store_path)
